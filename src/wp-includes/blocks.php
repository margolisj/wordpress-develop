...
function insert_hooked_blocks( &$parsed_anchor_block, $relative_position, $hooked_blocks, $context ) {
	...
	/*
	 * Read through the array cast rather than indexing into `attrs` directly. This function
	 * runs on the empty-object-preserving parse path, where `"metadata":{}` arrives as an
	 * empty stdClass -- and in PHP every array offset against an object is fatal, including
	 * the ones inside isset() and ??.
	 */
...
function set_ignored_hooked_blocks_metadata( &$parsed_anchor_block, $relative_position, $hooked_blocks, $context ) {
	...
	/*
	 * Rebuild `metadata` through an array cast rather than writing into it in place. This
	 * function runs on the empty-object-preserving parse path, where `"metadata":{}` arrives
	 * as an empty stdClass -- and in PHP every array offset against an object is fatal,
	 * including the ones inside isset() and ??, and including assignment.
	 */
	$anchor_metadata                  = (array) ( $parsed_anchor_block['attrs']['metadata'] ?? array() );
