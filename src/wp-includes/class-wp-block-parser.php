...
class WP_Block_Parser {
	 ...
	 *
	 * Kept separate from {@see WP_Block_Parser::parse()} so that the long-standing
	 * one-argument signature is untouched: a custom parser supplied through the
	 * `block_parser_class` filter may subclass this class and override `parse()`,
	 * and PHP rejects an override that declares fewer parameters than its parent.
	 * Delegating to `parse()` rather than reimplementing it also means such an
	 * override still runs when core asks for options.
	 *
	 * @since 7.1.0
	 *
	 * @param string $document Input document being parsed.
	...
	public function parse_with_options( $document, $options = array() ) {
		...
		try {
			...
			// Options scope to a single parse; a later parse() must not inherit them.
			$this->options = array();
		}
	}
	 ...
	 *
	 * Every other JSON object becomes a PHP array, exactly as the default parse path
	 * produces. That is what keeps the change safe: an empty object holds no keys and
	 * no strings, so nothing downstream needs to read into it or sanitize it, whereas a
	 * populated object would hide its contents from code that walks arrays.
	 *
	 * The distinction survives serialization on its own, because wp_json_encode() emits
	 * `{}` for an empty object and `[]` for an empty array. No marker and no restoration
	 * step is involved.
	 *
	 * @since 7.1.0
	 *
